...
impl<'a> StatusCreator<'a> {
    ...
    pub fn add_all(&mut self, ps: &Option<Vec<ProductIdT>>, status: &'static str) {
        for r in ps.iter().flatten() {
            let mut product = ProductStatus::default();
            ...
            match self.cache.get_relationship(&r.0) {
                Some(rel) => {
                    ...
                    // Find all products
                    product = self.cache.trace_product(&context.0).iter().fold(
                        product,
                        |mut product, branch| {
                    ...
                    product = self.cache.trace_product(&inner_id.0).iter().fold(
                        product,
                        |mut product, branch| {
                            product.update_from_branch(branch);
                            product
                ...
                None => {
                    ...
                    product = self.cache.trace_product(&r.0).iter().fold(
                        product,
                        |mut product, branch| {
                            product.update_from_branch(branch);
                            product
    ...
    pub async fn create<TX: AsRef<Transactional>>(
        ...
        product_status::Entity::insert_many(product_statuses)
            .exec(connection)
            .await?;
